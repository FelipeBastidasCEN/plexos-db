...
class DuckDBBulkLoader:
    ...
    def __init__(self, connection: duckdb.DuckDBPyConnection, chunk_size: int = 50_000):
        """
    ...
    def _prepare_table(self, table_name: str) -> None:
        ...
        sql = f"""
            ...
            ({", ".join([f'"{col}"' if col.lower() == "index" else col 
    ...
    def get_stats(self) -> dict[str, int]:
        ...
        """
        return {
            table_name: len(buffer) 
            for table_name, buffer in self._buffers.items()
        }
