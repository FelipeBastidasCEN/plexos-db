...
class ConfigService:
    ...
    DEFAULT_CONFIG = {
        "xml": {"default_name": "SolutionDataset.xml", "encoding": "utf-8"},
        "database": {
            "default_chunk_size": 10_000,
            "max_chunk_size": 1_000_000,
            "schema_name": "plexos",
        },
        "processing": {"max_memory_mb": 1024, "timeout_seconds": 300},
        "logging": {
            "level": "INFO",
            "console": True,
            "file": True,
    ...
    def _load_config(self) -> None:
        ...
        try:
            ...
            if self.config_path and self.config_path.suffix.lower() in [
                ".yaml",
                ".yml",
            ]:
                ...
                raise ConfigurationError(
                    f"Formato de configuración no soportado: {self.config_path}"
                )
    ...
    def set(self, key_path: str, value: Any) -> None:
        ...
        try:
            ...
            raise ConfigurationError(
                f"Error estableciendo configuración {key_path}: {e}"
            )
    ...
    def validate_chunk_size(self, chunk_size: int) -> None:
        ...
        if chunk_size < min_size:
            raise ConfigurationError(
                f"chunk_size debe ser >= {min_size}, recibido: {chunk_size}"
            )

        if chunk_size > max_size:
            raise ConfigurationError(
                f"chunk_size debe ser <= {max_size}, recibido: {chunk_size}"
            )
